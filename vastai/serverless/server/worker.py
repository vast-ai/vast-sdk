...
ClientResponseGenerator = Callable[
    ...
    def _build_handlers(self) -> None:
        ...
        if not self.config.handlers:
            default_handler_config = HandlerConfig(
                route="/", healthcheck=self.config.model_healthcheck_url
            )
    ...
    def _create_handler(self, handler_config: HandlerConfig) -> EndpointHandler:
                    ...
                    if benchmark_config:
                        if benchmark_config.dataset:
                            ...
                            try:
                                ...
                                raise (
                                    Exception(
                                        f'Error generating benchmark data for "{handler_config.route}" handler: {ex}'
                                    )
                                )
                    else:
                        raise (
                            Exception(
                                f'Error generating benchmark data for "{handler_config.route}" handler: Missing BenchmarkConfig!'
                            )
                        )
                ...
                def from_json_msg(cls, json_msg: Dict[str, Any]) -> "GenericApiPayload":
                    if not isinstance(json_msg, dict):
                        raise JsonDataException(
                            {"data": "payload must be a dictionary"}
                        )
        ...
        class GenericEndpointHandler(EndpointHandler[PayloadClass]):
            ...
            has_benchmark: bool = field(
                default=(True if handler_config.benchmark_config else False)
            )
            allow_parallel_requests: bool = field(
                default=handler_config.allow_parallel_requests
            )
            max_queue_time: float = field(default=handler_config.max_queue_time)
            ...
            concurrency: int = field(
                default=(
                    ...
                    if handler_config.benchmark_config
                    and handler_config.benchmark_config.concurrency
                    else 10
                )
            )
            ...
            async def call_remote_dispatch_function(self, params: dict):
                ...
                if self.remote_function is None:
                    raise RuntimeError(
                        f"remote_function is not configured for route {self._route}"
                    )
                ...
                try:
                    ...
                    raise RuntimeError(
                        f"Error calling remote function for route {self._route}: {ex}"
                    ) from ex
            ...
            async def generate_client_response(
                ...
                if user_response_generator:
                    try:
                        return await user_response_generator(
                            client_request, model_response
                        )
                    except Exception as e:
                ...
                is_stream = (
                    ...
                    or content_type in ("application/x-ndjson", "application/jsonl")
        ...
        for handler in self._handlers.values():
            if handler.has_benchmark:
                if benchmark_handler is not None:
                    raise Exception(
                        "Cannot define BenchmarkConfig for more than one EndpointHandler!"
                    )
                else:
