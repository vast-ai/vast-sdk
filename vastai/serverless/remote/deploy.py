import inspect
import json
from typing import Optional, Any, Callable, Awaitable, ParamSpec, TypedDict, Unpack
...
class Deployment(Deployment_):  # TODO: Async Context Manager compatible with client
    ...
    def _into_deployment_config_and_tarball(self) -> tuple[DeploymentConfig, str]:
        ...
        hash, size, tar_path = self._compute_hash_filesize_and_tarball(
    ...
    def _compute_hash_filesize_and_tarball(
        self, checked_name: str, checked_image: Image
    ) -> tuple[str, int, str]:
        ...
        tar_path = create_deployment_tarball(
            config, self._deployment_file, checked_image.copies
        )
